...
export class NewsService {
  ...
  protected convertDateFromClient(news: INews): INews {
    const copy: INews = Object.assign({}, news, {
      creationDate: news.creationDate && news.creationDate.isValid() ? news.creationDate.toJSON() : undefined,
      modificationDate: news.modificationDate && news.modificationDate.isValid() ? news.modificationDate.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((news: INews) => {
        news.creationDate = news.creationDate ? moment(news.creationDate) : undefined;
        news.modificationDate = news.modificationDate ? moment(news.modificationDate) : undefined;
      });
    }
    return res;
  }
