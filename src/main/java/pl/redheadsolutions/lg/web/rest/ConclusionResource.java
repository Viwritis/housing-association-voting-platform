...
/**
 * REST controller for managing {@link pl.redheadsolutions.lg.domain.Conclusion}.
 */
@RestController
@RequestMapping("/api")
public class ConclusionResource {
    ...
    public ResponseEntity<Conclusion> createConclusion(@Valid @RequestBody Conclusion conclusion) throws URISyntaxException {
    ...
    public ResponseEntity<Conclusion> updateConclusion(@Valid @RequestBody Conclusion conclusion) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, conclusion.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<List<Conclusion>> getAllConclusions(Pageable pageable) {
        ...
        Page<Conclusion> page = conclusionService.findAll(pageable);
